...
class ChatGPTService {
  ...
  static Future<Map<String, dynamic>> generatePlantContent(
    String plantName,
    String species, {
    String? base64Image,
  }) async {
  ...
  static Future<Map<String, dynamic>> _generatePlantContentWithRetry(
    String base64Image,
    ...
    for (int attempt = 0; attempt <= maxRetries; attempt++) {
      ...
      try {
        ...
        if (attempt > 0) {
          promptToUse =
              '''CRITICAL: You MUST provide ALL required measurements EXACTLY in this format:
...
HEALTH ASSESSMENT: [Is it healthy? Any problems?]''';
        ...
        final response = await http.post(
          ...
          body: jsonEncode({
            ...
            'temperature': attempt == 0
                ? 1.0
                : 0.3, // Lower temperature on retry for more consistent formatting
          }),
        );
        ...
        if (response.statusCode == 200) {
          ...
          if (recommendations.containsKey('amount_ml') &&
              recommendations['amount_ml'] != null) {
            print(
              '✅ Plant content generation successful with measurements on attempt ${attempt + 1}',
            );
            return recommendations;
          } else {
            print(
              '⚠️ Attempt ${attempt + 1} failed: Scientific calculation returned null',
            );
            if (attempt < maxRetries) {
              ...
              throw Exception(
                'Failed to extract required measurements after $maxRetries retries. Please try again or provide more detailed plant information.',
              );
            }
          }
        } else {
          print(
            '❌ gpt-4o API request failed with status: ${response.statusCode}',
          );
          throw Exception(
            'Failed to generate plant content: ${response.statusCode}',
          );
        }
      } catch (e) {
        if (attempt == maxRetries) {
    ...
    throw Exception(
      'Failed to generate plant content after $maxRetries retries',
    );
  }

  /// Analyzes plant health using OpenAI's GPT-4 Vision API
  static Future<Map<String, dynamic>> analyzePlantHealth(
    String base64Image,
    String prompt,
  ) async {
    try {
  ...
  static Map<String, dynamic> _parseAIResponse(String aiResponse) {
    try {
      ...
      if (response.contains('low light') || response.contains('shade')) {
        light = 'Low light';
      } else if (response.contains('direct sun') ||
          response.contains('full sun')) {
      ...
      if (response.contains('daily') ||
          response.contains('every day') ||
          response.contains('1 day')) {
        wateringFrequency = 1;
      } else if (response.contains('every 2 days') ||
          response.contains('2 days')) {
        wateringFrequency = 2;
      } else if (response.contains('every 3 days') ||
          response.contains('3 days')) {
        wateringFrequency = 3;
      } else if (response.contains('every 4 days') ||
          response.contains('4 days')) {
        wateringFrequency = 4;
      } else if (response.contains('every 5 days') ||
          response.contains('5 days')) {
        wateringFrequency = 5;
      } else if (response.contains('every 6 days') ||
          response.contains('6 days')) {
        wateringFrequency = 6;
      } else if (response.contains('every 7 days') ||
          response.contains('7 days') ||
          response.contains('weekly')) {
        wateringFrequency = 7;
      } else if (response.contains('every 10 days') ||
          response.contains('10 days')) {
        wateringFrequency = 10;
      } else if (response.contains('every 14 days') ||
          response.contains('14 days') ||
          response.contains('biweekly')) {
        wateringFrequency = 14;
      } else if (response.contains('every 21 days') ||
          response.contains('21 days') ||
          response.contains('3 weeks')) {
        wateringFrequency = 21;
      } else if (response.contains('monthly') ||
          response.contains('every month') ||
          response.contains('30 days')) {
      ...
      final careRecommendations = _extractStructuredCareRecommendations(
        aiResponse,
      );

      // Extract fields for scientific watering calculation
      final scientificWatering = _calculateScientificWatering(
        aiResponse,
        plantName,
      );
      ...
      if (scientificWatering != null) {
        print(
          '✅ Scientific watering calculated: ${scientificWatering['amount_ml']} ml, mode: ${scientificWatering['mode']}',
        );
      } else {
  ...
  static String _extractStructuredCareRecommendations(String response) {
    ...
    for (final line in lines) {
      ...
      if (lowerLine.contains('interesting facts') ||
          lowerLine.contains('fun facts')) {
        break;
      }
  ...
  static String _extractIssues(String response) {
    ...
    if (response.toLowerCase().contains('yellow') ||
        response.toLowerCase().contains('yellowing')) {
      issues.add('Yellowing leaves');
    }
    if (response.toLowerCase().contains('brown') ||
        response.toLowerCase().contains('browning')) {
      issues.add('Brown spots or edges');
    }
    if (response.toLowerCase().contains('wilted') ||
        response.toLowerCase().contains('wilting')) {
      issues.add('Wilting or drooping');
    }
    if (response.toLowerCase().contains('dry') ||
        response.toLowerCase().contains('underwatered')) {
      issues.add('Underwatering');
    }
    if (response.toLowerCase().contains('wet') ||
        response.toLowerCase().contains('overwatered')) {
  ...
  static Map<String, dynamic>? _calculateScientificWatering(
    String aiResponse,
    String plantName,
  ) {
    try {
      ...
      final potPresentMatch = RegExp(
        r'Pot\s*Present:\s*(yes|no)',
        caseSensitive: false,
      ...
      double? potHeight;
      if (hasPot) {
        final diameterMatch = RegExp(
          r'Pot\s*Diameter:\s*(\d+(?:\.\d+)?)\s*(cm|in)',
          caseSensitive: false,
        ).firstMatch(aiResponse);
        ...
        print(
          '🔍 Pot dimensions: diameter=$potDiameter cm, height=$potHeight cm',
        );
      }

      // Extract plant dimensions
      final plantHeightMatch = RegExp(
        r'Plant\s*Height:\s*(\d+(?:\.\d+)?)\s*(cm|in)',
        caseSensitive: false,
      ).firstMatch(aiResponse);
      double? plantHeight;
      ...
      final canopyMatch = RegExp(
        r'Canopy\s*Diameter:\s*(\d+(?:\.\d+)?)\s*(cm|in)',
        caseSensitive: false,
      ).firstMatch(aiResponse);
      ...
      print(
        '🔍 Plant dimensions: height=$plantHeight cm, canopy=$canopyDiameter cm',
      );

      // Extract soil state
      final soilStateMatch = RegExp(
        r'Visual\s*Soil\s*State:\s*(wet|moist|slightly_dry|dry|very_dry|not_visible)',
        caseSensitive: false,
      ).firstMatch(aiResponse);
      ...
      final profileMatch = RegExp(
        r'Plant\s*Profile:\s*(succulent|succulent_large|tropical_broadleaf|herbaceous|woody_potted|large_palm_indoor)',
        caseSensitive: false,
      ).firstMatch(aiResponse);
      ...
      if (!hasPot && (plantHeight == null || canopyDiameter == null)) {
        print(
          '⚠️ Insufficient data for scientific watering calculation - no pot and missing plant dimensions',
        );
        return null;
      }
      ...
      if (hasPot && (potDiameter == null || potHeight == null)) {
        print(
          '⚠️ Insufficient data for scientific watering calculation - has pot but missing pot dimensions',
        );
        return null;
      }
      ...
      if (hasPot && potDiameter != null && potHeight != null) {
        ...
        effectiveVolumeMl = WateringCalculatorService.calculatePotVolume(
          container,
        );
        ...
        effectiveVolumeMl =
            WateringCalculatorService.calculateEquivalentRootVolume(plantDims);
      } else {
      ...
      // Parse profile
      PlantProfile profile = WateringCalculatorService.parsePlantProfile(
        plantName,
        null,
      );
      if (profileText != null) {
  ...
  static List<String> _extractInterestingFacts(String response) {
    ...
    for (final line in lines) {
      ...
      if (lowerLine.contains('interesting facts') ||
          lowerLine.contains('fun facts') ||
          lowerLine.contains('4. interesting facts') ||
          lowerLine.contains('4. interesting facts:')) {
      ...
      if (inInterestingFactsSection &&
          (lowerLine.contains('care recommendations') ||
              ...
              lowerLine.contains('2.') ||
              lowerLine.contains('3.') ||
              lowerLine.contains('5.'))) {
        break;
      }
  ...
  static String _cleanSectionTitle(String title) {
    ...
    String cleanTitle = title
        .replaceAll(
          RegExp(r'^[-*\s]+'),
          '',
        ) // Remove leading dashes, asterisks, spaces
        .replaceAll(
          RegExp(r'[-*\s]+$'),
          '',
        ) // Remove trailing dashes, asterisks, spaces
        .trim();
  ...
  static List<String> _extractCareSectionsFromText(String response) {
    ...
    if (lowerResponse.contains('water') || lowerResponse.contains('watering')) {
      sections.add(
        'Watering: Monitor soil moisture regularly and water when the top inch feels dry',
      );
    }

    if (lowerResponse.contains('light') || lowerResponse.contains('sun')) {
      sections.add(
        'Light Requirements: Ensure proper light conditions for optimal growth',
      );
    }

    if (lowerResponse.contains('temperature') ||
        lowerResponse.contains('temp')) {
    ...
    if (lowerResponse.contains('soil') || lowerResponse.contains('potting')) {
      sections.add(
        'Soil: Use well-draining potting mix appropriate for the plant type',
      );
    }

    if (lowerResponse.contains('fertilizer') ||
        lowerResponse.contains('fertilizing')) {
      sections.add(
        'Fertilizing: Apply appropriate fertilizer during growing season',
      );
    }

    if (lowerResponse.contains('humidity') ||
        lowerResponse.contains('moisture')) {
      sections.add(
        'Humidity: Consider humidity levels for optimal plant health',
      );
    }

    if (lowerResponse.contains('growth') || lowerResponse.contains('size')) {
      sections.add(
        'Growth Rate / Size: Monitor growth patterns and provide adequate space',
      );
    }

    if (lowerResponse.contains('bloom') || lowerResponse.contains('flower')) {
      sections.add(
        'Blooming: Provide proper care to encourage flowering when applicable',
      );
    }
  ...
  static List<String> _extractFactsFromResponse(String response) {
    ...
    if (lowerResponse.contains('native to') ||
        lowerResponse.contains('origin')) {
      final match = RegExp(
        r'native to ([^.]+)',
        caseSensitive: false,
      ).firstMatch(response);
    ...
    if (lowerResponse.contains('attracts') ||
        lowerResponse.contains('pollinators')) {
      facts.add('Attracts beneficial pollinators and wildlife');
    }

    if (lowerResponse.contains('medicinal') ||
        lowerResponse.contains('health benefits')) {
    ...
    if (lowerResponse.contains('variety') ||
        lowerResponse.contains('cultivar')) {
      facts.add('Comes in many beautiful varieties and cultivars');
    }

    if (lowerResponse.contains('seasonal') ||
        lowerResponse.contains('annual')) {
