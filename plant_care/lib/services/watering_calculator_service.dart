...
enum VisualSoilState { wet, moist, slightlyDry, dry, veryDry, notVisible }
...
class ContainerDimensions {
  ...
  ContainerDimensions({required this.potDiameterCm, required this.potHeightCm});
}
...
class WateringCalculatorService {
  ...
  static WateringResult calculateWatering({
    ...
    print(
      '🌱 Watering Calculator: profile=$profile, soil=$soilState, volume=${effectiveVolumeMl.toInt()}ml',
    );

    // Handle wet/moist soil - recheck only
    if (soilState == VisualSoilState.wet ||
    ...
    return WateringResult(
      ...
      mode: 'after_watering',
      reason:
          '$profile: ${amountMl}ml (${effectiveVolumeMl.toInt()}ml vol × ${baseFractions[0]}-${baseFractions[1]} × ${soilMult} soil)',
    );
  }
  ...
  static PlantProfile parsePlantProfile(String? aiName, String? species) {
    ...
    // Large cacti
    if (name.contains('saguaro') ||
        name.contains('organ pipe') ||
        name.contains('prickly pear')) {
    ...
    if (name.contains('palm') ||
        name.contains('areca') ||
        name.contains('fan palm')) {
      return PlantProfile.largePalmIndoor;
    }

    // Woody/trees
    if (name.contains('tree') ||
        name.contains('bonsai') ||
        species?.toLowerCase().contains('tree') == true) {
    ...
    if (name.contains('monstera') ||
        ...
        name.contains('fiddle leaf') ||
        name.contains('bird of paradise') ||
  ...
  static VisualSoilState parseSoilState(String? description) {
    ...
    if (desc.contains('wet') || desc.contains('soggy'))
      return VisualSoilState.wet;
    if (desc.contains('moist') || desc.contains('damp'))
      return VisualSoilState.moist;
    if (desc.contains('slightly dry') || desc.contains('somewhat dry'))
      return VisualSoilState.slightlyDry;
    if (desc.contains('very dry') || desc.contains('extremely dry'))
      return VisualSoilState.veryDry;
  ...
  static ContainerDimensions parsePotDimensions(
    String? potSizeText,
    String? plantSizeText,
  ) {
    if (potSizeText == null ||
        potSizeText == 'none' ||
        potSizeText == 'unknown') {
