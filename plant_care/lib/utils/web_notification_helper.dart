...
class WebNotificationHelper {
  ...
  static Map<String, dynamic> getNotificationSupport() {
    if (!kIsWeb) {
      return {'supported': false, 'reason': 'Not running on web platform'};
    }
  ...
  static Future<Map<String, dynamic>> requestNotificationPermission() async {
    if (!kIsWeb) {
      return {'success': false, 'error': 'Not running on web platform'};
    }
    ...
    try {
      ...
      if (!support['supported']) {
        return {
          'success': false,
          'error':
              'Notifications not supported: ${support['reason'] ?? 'Unknown reason'}',
          'support': support,
        };
      }
