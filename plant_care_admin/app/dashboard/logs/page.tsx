...
export default function LogsPage() {
  ...
  useEffect(() => {
    Promise.all([fetchMailLogs(200), fetchFcmTokens(), fetchUsers(), fetchHealthChecks(300), fetchAiUsage(500)])
      .then(([l, t, u, hc, ai]) => {
        setLogs(l);
        setTokens(t);
  ...
  const aiSummary = summarizeAiUsage(aiRecords);
      ...
      <Tabs defaultValue="emails">
        ...
        <TabsContent value="ai" className="space-y-4 mt-4">
          <div className="grid grid-cols-4 gap-4">
            <StatCard icon={Sparkles} value={aiRecords.length} label="Total AI calls" color="bg-purple-100 text-purple-600" />
            <StatCard icon={Users} value={new Set(aiRecords.filter((r) => r.userId).map((r) => r.userId!)).size} label="Users using AI" color="bg-blue-100 text-blue-600" />
            <Card>
              <CardContent className="pt-4 pb-4 flex items-center gap-3">
                ...
                <div>
                  <p className="text-2xl font-bold">${aiSummary.totalCostUsd.toFixed(4)}</p>
                  ...
                  {aiRecords.length === 0 ? (
                    <TableRow>
                      <TableCell colSpan={7} className="py-10 text-center text-muted-foreground">
                        No AI usage records yet. They will appear after the first AI call.
                      </TableCell>
                    </TableRow>
                  ) : (
                    aiRecords.map((r) => {
